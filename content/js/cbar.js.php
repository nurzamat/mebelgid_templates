...
function cbar_rule_active (rule_id, track_time){ // store rules on cookie inc engagement time and page views
	...
	if (cbar_rules_str!=null){
		...
		for ( rule = rules.length-1; rule  >=0; rule--) {
			
			var ruleparts=rules[rule].split('~');
			
			if ( (cbar_sess -  parseInt(ruleparts[3])) > 10 ) { // forget old rules
				rules.splice(rule,1);
				continue; 
			}
			
			if ( rules[rule].indexOf(rule_id+'~') >-1) {
				rule_found=true;
...
function cbar_div_popup_show (rule_id, popup_width, popup_height, popup_top, img_url, html, img_is_background, img_width, img_height , token, replace_encoded, embed_data, popup_close, border, bordercol, backg, darken, corner){//(called from data.js)
...
function cbar_iframe_popup_show (rule_id, popup_url, popup_width , popup_height ,popup_top, img_w, img_h, img_url, popup_close){//(called from data.js)
...
function cbar_trust_message (payload){
	...
	var questionmark="<a class='cbar_trust_qm' style='float:right; font-size:80%; opacity:0.5;  cursor:pointer; cursor:hand;' onclick='cbar_trust_popup(); return false;'>?</a>";
	...
	if (typeof msg_data.embed_pt != 'undefined') {
		...
		for (var index in msg_data.message) {
			if (msg_data.message.hasOwnProperty(index)) {
				 op=$("[cbar_live_insert]:eq("+index+")").attr('cbar_live_insert');
				 
				 // prod page
				 
					var msg = "<div style='"+cbar_trust_outdiv_css+"'><span style='z-index:10;"+msg_data.embedcss+"'>"+msg_data.message[index]+questionmark+"</span></div>";
//				else
	//				var msg = "<span style='"+msg_data.embedcss+"'>"+msg_data.message[index]+ questionmark+"</span>";
				 
				 if (op == 'append') $("[cbar_live_insert]:eq("+index+")").append(msg);
				 if (op == 'prepend') $("[cbar_live_insert]:eq("+index+")").prepend(msg);
				 if (op == 'before') $("[cbar_live_insert]:eq("+index+")").before(msg);
				 if (op == 'after') $("[cbar_live_insert]:eq("+index+")").after(msg);
			}
		}
	...
	if (msg_data.type=='bubble' || msg_data.type=='side') {
		...
		div.innerHTML = "<div class='cbar_bubble' id='cbar_trust_"+msg_data.rule_id+"_"+msg_data.count+"' style='position:fixed; width:"+msg_data.width+"px; height:"+height+"; right:"+right+"; left:"+left+"; top:"+top+"; bottom:"+bottom+"; opacity:0.1; background:"+msg_data.bg+";   color:"+msg_data.txtcolor+"; border: "+msg_data.borderw+"px solid "+msg_data.border+"; border-radius:"+msg_data.round+"px; padding:5px; box-shadow: "+msg_data.shadow+"px "+msg_data.shadow+"px "+msg_data.shadow+"px #888888; text-align:"+msg_data.align+";  font:"+msg_data.font+"; z-index:2000000; "+msg_data.extracss+"'>"+content+"</div>";
	} 
	else  // top bar
		div.innerHTML = "<div class='cbar_bubble' id='cbar_trust_"+msg_data.rule_id+"_"+msg_data.count+"' style=' position:fixed; width:100%; height:"+height+"; right:"+right+"; left:"+left+"; top:"+top+"; bottom:"+bottom+"; opacity:0.1; background:"+msg_data.bg+"; "+dynamic_css+"   color:"+msg_data.txtcolor+";  border-radius:0px; padding:10px;  text-align:center;  font:"+msg_data.font+"; z-index:2000000; "+msg_data.extracss+"'><div style='margin:"+msg_data.barmargin+"px;'>"+msg_data.message[0]+" <a class='cbar_trust_qm' style='margin-left:30px; font-size:80%; opacity:0.5;  cursor:pointer; cursor:hand;' onclick='cbar_trust_popup(); return false;'>?</a></div></div>";
...
function cbar_banner_show (img_url, width, height, clk_url, align, position, lrmargin, tbmargin, html_item , zindex, overlay, newwin, banner_id, close_butt_pos, onclick, mouseover,mouseout, rule_id, extra_html){//(called from data.js)
	...
	if ( (position=='html') && ((overlay =='r') || (overlay =='rp')) ) return; // overwrite existing HTML element content
...
function cbar_start () {
	...
	cbar_sid 			= 47468; 		//determine the SID using QWERTY format using first 5 chars of the domain name
...
cbar_cookiedomain	= '.furnitureonline.com.au';	//replace  domain.com with actual site domain	
...
if(document.location.hostname.toLowerCase().search('furnitureonline.com.au') == -1) cbar_ro = true;
...
//standard flow
//cbar_flow="H0:1,3-12,13,5.9,12,8,3;H1:1,0,0,0,0;P0:1,2,13,1,5,2;P1:1,2,4.4,1.4.0.11,2;C0:1,2-9,13,5.0,2.0,3;C1:1,0,0,0,0;Sc0:1,2,7.4.0.0.7,7.2.0.0.1,11.4;Sc1:1,0,0,0,0";
...
var cbar_temp = cbar_Sizzle ('body.catalog-category-view'); 
if (cbar_temp.length >0 && cbar_url.toLowerCase().search('/sale') == -1 && cbar_url.toLowerCase().search('/collections') == -1) 
{
...
cbar_temp = cbar_Sizzle ('div.main div.product-view');
if (cbar_temp.length>0)
{
	// Cbar Pid. scrapped generally from URL. Please make sure PIDs are always lowerCased if they are alpha-numeric
	cbar_temp = cbar_Sizzle('div.product-shop input[name="product"]');
	if (cbar_temp.length>0){ 
		cbar_pid = cbar_temp[0].value.replace(/^\s+|\s+$/g,"");
	}
	...
	//name	
	cbar_temp = cbar_Sizzle('div.product-shop-holder div.product-name h1');
	if (cbar_temp.length>0) {
	...
	cbar_temp = document.location.pathname.split("/");
	if(cbar_url.toLowerCase().search('/sale') == -1 && cbar_url.toLowerCase().search('/collections') == -1 && cbar_temp.length>2){
		tmp_len = cbar_temp.length;
		cbar_cid = cbar_temp[tmp_len - 2];
		cbar_cname = cbar_cid;
		cbar_cid = cbar_cid.toLowerCase();	
	}
	...
	//out of stock. 
	cbar_temp= cbar_Sizzle ('div.ProductMain div.OutOfStockMessage div.CurrentlySoldOut p:first');
	if (cbar_temp.length>0 && cbar_temp[0].innerHTML.toLowerCase().replace(/^\s+|\s+$/g,"") == "sorry but this item is currently unavailable."){
		cbar_oos = 1;
	}
	...
	var cbar_temp = cbar_Sizzle('div.product-view div.product-img-box a.MagicZoomPlus img[itemprop="image"]'); //generally from main image tag
	if (cbar_temp.length>0) {
	...
	//Product Price
	cbar_temp = cbar_Sizzle('div.product-price-box div.price-box span.regular-price span.price, div.product-price-box div.price-box p.special-price span.price');
	if (cbar_temp.length>0){ 
	...
	var cbar_temp= cbar_Sizzle('div.product-price-box div.price-box p.old-price span.price');
	if (cbar_temp.length>0) {
...
if ((cbar_url.toLowerCase().search('/catalogsearch/result')>-1) && (cbar_url.toLowerCase().search('q=')> -1)){
	...
	temp=cbar_gup('q').replace(/\+/," ");
	cbar_search=temp;	//scrape the search keyword. generally from url parameter
}
...
if (cbar_url.toLowerCase().search('/checkout/cart')>-1) {
	...
	var cbar_temp = cbar_Sizzle('table#shopping-cart-table h2.product-name a');
	...
	var cbar_temp = cbar_Sizzle('table#shopping-cart-table td.a-center input.qty');
	for (var nodei=0; nodei < cbar_temp.length; nodei++){
	...
	cbar_temp =cbar_Sizzle ('input[id="coupon_code"]');
	if (cbar_temp.length>0){
...
cbar_after_cou_inj = function (input){	
	...
	tmp = cbar_Sizzle('div.discount button.button');
	if(tmp.length>0){		
			jQuery(tmp[0].click());  //use jquery so it works on safari.
	}		
}	
//end coupon code callbacks


cbar_watch_email_container = function()
{
	cbar_temp=cbar_Sizzle("div#CheckoutStepBillingAddress div.ExpressCheckoutContent");
	if(cbar_temp.length>0)
	{
		if(cbar_temp[0].style.display=="block"){
			if(typeof(cbar_email_scraping_timer)!='undefined') window.clearInterval(cbar_email_scraping_timer);
			
			cbar_alreadyrunflag=false;
			cbar_start("scrape_email");
		}
	}
}

cbar_scrape_email_again=function(){

	cbar_temp= cbar_Sizzle("input[id='login_email']");
	if(cbar_temp.length>0){
		if(cbar_e_input){
	...
	cbar_temp= cbar_Sizzle("input[id='FormField_1']");
	if(cbar_temp.length>0){
		if(cbar_e_input){
			cbar_e_input.push(cbar_temp[0]);
		}
...
if(cbar_url.toLowerCase().search('/checkout/onepage') > -1){
	cbar_e_input = cbar_Sizzle('input[id="login:email"], input#login-email');
	cbar_e_com = true;
}

cbar_e_input = cbar_e_input.concat(cbar_Sizzle('input#magestore-sociallogin-popup-email, input[id="socialogin.email"]'));
if(cbar_e_input.length>0)
	cbar_e_com = true;


//end email

//scrape first name in token.
jQuery('input#FormField_4,input#nrlsk_FNAME,input#nl_first_name').blur(function(){  
	cbar_alreadyrunflag=false;
	cbar_start();
})

cbar_temp = cbar_Sizzle('input#FormField_4,input#nrlsk_FNAME,input#nl_first_name')
for (var nodei=0; nodei < cbar_temp.length; nodei++)
{
	cbar_temp1= cbar_temp[nodei].value;
	if(cbar_temp1!="")
		cbar_userinfo.first_name = cbar_temp1;
}

// Thank You Page scraping. (possible only after purchase is made)
if  (cbar_url.search('finishorder.php')>-1 || cbar_url.search('checkout/onepage/success/')>-1) {  
